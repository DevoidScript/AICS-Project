/**
 * Offline eligibility — caches all Google Sheets records locally for offline cooldown checks.
 * Fetches via ?action=getAllRecords (3 fields only: patientName, date, typeOfAssistance).
 * Refreshes every 30 minutes. When offline, checkEligibilityOffline() runs cooldown math on cache.
 * If cache is empty and staff is offline, submission is blocked (no override).
 */
(function() {
  var STORAGE_KEY = "aicsRecordCache";
  ...
  function fetchAllRecords(callback) {
    var url = (typeof APPS_SCRIPT_URL !== "undefined" ? APPS_SCRIPT_URL : window.APPS_SCRIPT_URL || "") + "?action=getAllRecords&callback=__aicsGetAllRecords" + Date.now() + "&_=" + Date.now();
    if (!url || url.indexOf("?action=getAllRecords") === -1) {
      if (callback) callback(null);
      return;
    }
    var callbackName = "__aicsGetAllRecords" + Date.now();
  ...
  window.AICS_checkEligibilityOffline = checkEligibilityOffline;
  window.AICS_getRecordCache = getCache;
  window.AICS_hasRecordCache = function() {
    var c = getCache();
    return !!(c && c.records && c.records.length > 0);
  };
  window.AICS_refreshRecordCache = fetchAllRecords;
